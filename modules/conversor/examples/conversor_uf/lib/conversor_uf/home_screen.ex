defmodule ConversorUf.HomeScreen do
  @moduledoc "Landing screen for ConversorUf."
  use Mob.Screen
  ...
  def render(assigns) do
    ...
    <Scroll background={:background}>
      <Column background={:background} padding={:space_lg}>
        ...
        <Spacer size={16} />
        <Text text="ConversorUf" text_size={:xl} text_color={:on_surface} padding={:space_sm} />
  ...
  def handle_info({:tap, :open_text}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, ConversorUf.TextScreen)}
  end

  def handle_info({:tap, :open_list}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, ConversorUf.ListScreen)}
  end

  def handle_info({:tap, :open_dice}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, ConversorUf.DiceScreen)}
  end

  def handle_info({:tap, :open_webview}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, ConversorUf.WebViewScreen)}
  end

  def handle_info({:tap, :open_audio}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, ConversorUf.AudioScreen)}
  end

  def handle_info({:tap, :open_camera}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, ConversorUf.CameraScreen)}
  end

  def handle_info({:tap, :open_storage}, socket) do
    {:noreply, Mob.Socket.push_screen(socket, ConversorUf.StorageScreen)}
  end
